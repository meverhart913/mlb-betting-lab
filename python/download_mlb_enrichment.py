...
def extract(gid, j):
    ...
    game_date = dt.get("officialDate") or str(dt.get("dateTime", ""))[:10]
    season = game.get("season")
    e = {
        "game_id": gid, "date": game_date, "season": season, "game_type": game.get("type"),
    ...
    for side in ["away", "home"]:
        ...
        b = ts.get("batting", {}) or {}
        p = ts.get("pitching", {}) or {}
        f = ts.get("fielding", {}) or {}
        ids = team_box.get("pitchers", []) or []
        starter = ids[0] if ids else None
        players = team_box.get("players", {}) or {}
        e[f"{side}_team_id"] = team.get("id")
        e[f"{side}_starter_id"] = starter
        e[f"{side}_starter_name"] = (players.get(f"ID{starter}", {}).get("person", {}) or {}).get("fullName") if starter else None
        trs.append({
            "game_id": gid, "date": game_date, "season": season, "side": side, "team_id": team.get("id"), "team_name": team.get("name"),
        ...
        for pid in ids:
            q = players.get(f"ID{pid}", {}) or {}
            st = q.get("stats", {}).get("pitching", {}) or {}
            person = q.get("person", {}) or {}
            prs.append({
                "game_id": gid, "date": game_date, "season": season, "side": side, "pitcher_id": pid, "pitcher_name": person.get("fullName"),
...
def main():
    ...
    a.add_argument("--input", type=Path, default=DATA / "mlb_games_2018_present.csv")
    a.add_argument("--outdir", type=Path, default=DATA)
    a.add_argument("--start-year", type=int, default=2018)
    a.add_argument("--end-year", type=int, default=date.today().year)
    a.add_argument("--limit", type=int)
    a.add_argument("--no-verify-ssl", action="store_true")
    a.add_argument("--sleep", type=float, default=.12)
    x = a.parse_args()

    out = x.outdir
    out.mkdir(parents=True, exist_ok=True)
    ep = out / "mlb_game_enrichment.csv"
    pp = out / "mlb_pitcher_game_logs.csv"
    tp = out / "mlb_team_game_logs.csv"
    fp = out / "enrichment_failures.csv"
    g = pd.read_csv(x.input, low_memory=False)
    ...
    done = set(pd.read_csv(ep, usecols=["game_id"]).game_id.astype(int)) if ep.exists() else set()
    g = g[~g.game_id.astype(int).isin(done)]
    if x.limit:
        g = g.head(x.limit)

    s = session(not x.no_verify_ssl)
    eb, pb, tb, fb = [], [], [], []
    total = len(g)
    print(f"{len(done):,} complete; {total:,} remaining")
    for i, r in enumerate(g.itertuples(index=False), 1):
        try:
            z = s.get(URL.format(int(r.game_id)), timeout=45)
            z.raise_for_status()
            e, p, t = extract(int(r.game_id), z.json())
            eb.append(e); pb += p; tb += t
        except Exception as ex:
            fb.append({"game_id": r.game_id, "date": r.date, "error": repr(ex)})
        if i % 100 == 0 or i == total:
            append(ep, eb); append(pp, pb); append(tp, tb); append(fp, fb)
            eb, pb, tb, fb = [], [], [], []
            print(f"{i:,}/{total:,} ({i/total:.1%})")
        time.sleep(x.sleep)
    print(f"Done. Enrichment files are in {out}.")
