...
No API key required. Normalizes the CSV into the historical market schema used by
backtest_pitcher_k_market.py so the complete V2.2 pricing/grading pipeline can be
smoke-tested without consuming paid historical-odds credits.
"""
...
OUT=ROOT/'data'/'market'/'pitcher_k_market_history.csv'
...
def main():
    ...
    z=pd.DataFrame({
        ...
        'side':side,
        'american_odds':pd.to_numeric(pick(raw,'price_american','american_odds','price'),errors='coerce'),
        'sportsbook':pick(raw,'bookmaker','bookmaker_title','sportsbook'),
        'event_id':pick(raw,'event_id'),
        'snapshot_time':pick(raw,'closing_at','recorded_at','snapshot_time'),
    })
    z=z[z.side.isin(['over','under'])].dropna(subset=['date','pitcher_name','line','american_odds'])
    OUT.parent.mkdir(parents=True,exist_ok=True); z.to_csv(OUT,index=False)
